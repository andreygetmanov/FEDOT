...
from examples.multi_modal_pipeline_genres import calculate_validation_metric, generate_initial_pipeline_and_data, prepare_multi_modal_data
...
def run_multi_modal_case(files_path, is_visualise=False, timeout=datetime.timedelta(minutes=2)):
    ...
    #prediction = pipeline.predict(predict_data, output_mode='probs')
    #prediction_probs = prediction.predict[0:107][1]
    err = calculate_validation_metric(prediction, test_num)


    print(f'F1 for validation sample is {err}')
    return err
...
if __name__ == '__main__':
    ...
    run_multi_modal_case('cases/data/mm_imdb_small', is_visualise=True)
